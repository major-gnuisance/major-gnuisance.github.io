...
** Utility
   ...
   *Quicksave* or *Quickload*. \\
   ...
   | Squad send/recall | {{{btn(touchpad)}}}         |
...
* Feature List
  ...
  - Quick access to special weapons with {{{btn(dpadl)}}} {{{btn(dpadr)}}}
  ...
  - Gyroscope aim
    ...
    - Still available when holding aim button ({{{btn(l2)}}})
  - Quickload and Quicksave with safety (hold to trigger) on {{{btn(trackpad)}}}
